...
/* Directives */
var appDirectives = angular.module('gestionepazienti.directives', []);
...
appDirectives.directive("digitalClock", function($timeout, dateFilter) {
	  return {
	    restrict: 'E',
	    link: function(scope, iElement) {
	      (function updateClock() {
	        iElement.text(dateFilter(new Date(), 'dd/MM/yyyy HH:mm:ss'));
...
appDirectives.directive('logo', function() {
	return {
		restrict : 'E',
		template: '<div class="logo"><span class="gestione">GestionePazienti</span><br><span class="pazienti">Piattaforma Covid-19 Piemonte</span></div>'
	};
});
...
appDirectives.directive('alertPanel', [function(){
	return {
	  ...
	  template: '<div class="alert alert-{{content.type}}" ng-if="content.message || message">'+
				...
				'  <div> <span ng-if="content.code">Code: {{content.code}} - </span>'+ 
				'  <span ng-if="content.detail && !content.detail.data.message" title="{{content.detail}}">{{content.detail}}</span>'+
				'  <span ng-if="content.detail.data.message" title="{{content.detail.data.message}}">{{content.detail.data.message}}</span>'+
				'  </div> '+
