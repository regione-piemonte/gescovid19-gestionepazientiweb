package it.csi.gestionepazienti.gestionepazientiweb.filter.gestionepazienti;
...
public class IrideIdAdapterFilter extends AbstractIrideIdAdapterFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain fchn)
		...
		if(devmode && hreq.getRequestURI().equals("/gestionepazientiweb/api/swagger.json")) { 
			fchn.doFilter(req, resp);
			return;
		}
		...
		if (hreq.getSession().getAttribute(IRIDE_ID_SESSIONATTR) == null) {
			...
			if (marker != null) {
				try {
					...
					if (!user.getEnabled()) {
						LOG.info("[IrideIdAdapterFilter::doFilter] gestionepazientiweb user " + userIdentifier+" is not enabled. return.");
