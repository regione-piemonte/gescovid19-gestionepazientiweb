...
public class IrideIdAdapterFilter extends AbstractIrideIdAdapterFilter implements Filter {
	...
	public static final String APPLICATION_DASHBOARD = "dashboardcovid";
		...
		if (hreq.getSession().getAttribute(IRIDE_ID_SESSIONATTR) == null) {
			...
			if (marker != null) {
				try {
					...
					UserLogged  user = loadUser(userIdentifier, identita.getNome(), identita.getCognome(),req,APPLICATION_DASHBOARD); 
					user.setRunnningApplication(APPLICATION_DASHBOARD);

					// verifica per l'applicazione dashboard covid
					user.setEnabled(
							GenericUtils.currentUserHasProfilo(user, 1)
							||
							GenericUtils.currentUserHasProfilo(user, 5)
							);
					...
					if (!user.getEnabled()) {
						LOG.info("[IrideIdAdapterFilter::doFilter] dashboardcovid user " + userIdentifier+" is NOT enabled");
						hresp.sendRedirect(accessdeniedUrl);
					}
					else {
						LOG.info("[IrideIdAdapterFilter::doFilter] dashboardcovid user " + userIdentifier+" is enabled");
