package it.csi.gestionepazienti.gestionepazientiweb.filter.visurapazienti;
...
public class IrideIdAdapterFilter extends AbstractIrideIdAdapterFilter implements Filter {
	...
	public static final String APPLICATION_VISURA_PAZIENTI = "visura pazienti";
		...
		if (hreq.getSession().getAttribute(IRIDE_ID_SESSIONATTR) == null) {
			...
			if (marker != null) {
				try {
					...
					LOG.info("[IrideIdAdapterFilter::doFilter] richiamo il loaduser per leggere info dell'utente loggato.");
					UserLogged  user = loadUser(userIdentifier, identita.getNome(), identita.getCognome(),req,APPLICATION_VISURA_PAZIENTI); 
					LOG.info("[IrideIdAdapterFilter::doFilter] richiamo il loaduser per leggere info dell'utente loggato.");
					user.setRunnningApplication(APPLICATION_VISURA_PAZIENTI);
					// verifica per l'applicazione visura
					user.setEnabled(
							GenericUtils.currentUserHasProfilo(user, GenericUtils.ID_PROFILO_SINDACO)
							)
							;
					...
					if (!user.getEnabled()) {
						LOG.info("[IrideIdAdapterFilter::doFilter] visurapazienti user " + userIdentifier+" is NOT enabled");
						hresp.sendRedirect(accessdeniedUrl);
					}
					else {
						LOG.info("[IrideIdAdapterFilter::doFilter] visurapazienti user " + userIdentifier+" is enabled");
	...
	public String getToken(HttpServletRequest httpreq) {
		...
		if (marker == null && devmode) {
			LOG.info("[IrideIdAdapterFilter::getToken] marker (AUTH_ID_MARKER) is null!");
			return getTokenDevMode(httpreq);
		} else {
			try {
				// gestione dell'encoding
				LOG.info("[IrideIdAdapterFilter::getToken] marker:" + marker);
				String decodedMarker = new String(marker.getBytes("ISO-8859-1"), "UTF-8");
				LOG.info("[IrideIdAdapterFilter::getToken] decodedMarker:" + decodedMarker);
